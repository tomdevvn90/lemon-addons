...
class ElementAddon_Template_Library_Base
{
    ...
    protected $api_url = 'https://tpllib.beplusthemes.com/wp-json/wp/v2/library-template?_embed=true&per_page=100&acf_format=standard&status=publish';
    protected $api_url_cate = 'https://tpllib.beplusthemes.com/wp-json/wp/v2/library_temp_cat?acf_format=standard';
    ...
    public function createTemplateTables()
    {
        ...
        if(defined('BTEP_TPL_DB_VER') && BTEP_TPL_DB_VER != get_option('BTEP_TPL_DB_VER',false)){
            ...
            update_option('BTEP_TPL_DB_VER',BTEP_TPL_DB_VER);
        }
    ...
    public function setTemplateDataToDB()
    {
        ...
        $demoData = $this->maping_demo_data();
    ...
    /**
     * @return array|mixed
     * retrieve element addon categories from remote server with api route
     */
    public function remote_get_demo_data_post()
    {
        ...
        foreach ($body as $i => $value) {
            $arr_data[$i]['demo_id']    = $value['id'];
            $arr_data[$i]['date']       = explode("T",$value['date'])[0];
            $arr_data[$i]['title']      = $value['title']['rendered'];
            $arr_data[$i]['short_desc'] = $value['content']['rendered'];
            $arr_data[$i]['is_pro']     = $value['acf']['is_pro'] ? 1 : 0;
            $arr_data[$i]['type']       = $value['acf']['type'];
            $arr_data[$i]['thumbnail']  = $value['_embedded']['wp:featuredmedia'][0]['source_url'];
            $arr_data[$i]['demo_url']   = $value['link'];
            $arr_data[$i]['json_url']   = $value['acf']['json_url'];
            $arr_data[$i]['term_id']    = $value['_embedded']['wp:term'][0][0]['id'];
        }
        return $arr_data;
    }

    public function remote_get_demo_data_cate()
    {
        $final_url = $this->api_url_cate;
        $response = wp_remote_get($final_url, ['timeout' => 60, 'sslverify' => false]);
        $body = wp_remote_retrieve_body($response);
        $body = json_decode($body, true);
        $arr_cate = [];
        foreach ($body as $i => $value) {
            $arr_cate[$i]['term_id']     = $value['id'];
            $arr_cate[$i]['name']        = $value['name'];
            $arr_cate[$i]['slug']        = $value['slug'];
            $arr_cate[$i]['description'] = '';
            $arr_cate[$i]['total']       = $value['count'];
            $arr_cate[$i]['image_url']   = $value['acf']['image_url'];
        }
        return $arr_cate;
    }

    public function maping_demo_data() {
        $demoDataPost = $this->remote_get_demo_data_post();
        $demoDataCat = $this->remote_get_demo_data_cate();

        if($demoDataPost && $demoDataCat){
            foreach ($demoDataCat as $key => $vl_cat) {
                foreach ($demoDataPost as  $vl_post) {
                    if($vl_cat['term_id'] == $vl_post['term_id']){
                        $demoDataCat[$key]['data'][] = $vl_post;
                    }
                }
            }
        }
